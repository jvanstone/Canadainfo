...
class Canada_Info_Dark_Mode {
	...
	public function editor_custom_color_variables() {
		...
		wp_enqueue_script(
			'candainfo-dark-mode-support-toggle',
		...
		wp_enqueue_script(
			'candainfo-editor-dark-mode-support',
			get_template_directory_uri() . '/assets/js/editor-dark-mode-support.js',
			array( 'candainfo-dark-mode-support-toggle' ),
			'1.0.0',
			true
		);
	}
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style( 'tt1-dark-mode', $url, array( 'candainfo-style' ), wp_get_theme()->get( 'Version' ) ); // @phpstan-ignore-line. Version is always a string.
	}
	...
	public function customize_controls_enqueue_scripts() {
		...
		wp_enqueue_script(
			'candainfo-customize-controls',
			get_template_directory_uri() . '/assets/js/customize.js',
			array( 'customize-base', 'customize-controls', 'underscore', 'jquery', 'candainfo-customize-helpers' ),
			'1.0.0',
			true
		);
	}
	...
	public function customizer_controls( $wp_customize ) {
		...
		if ( is_object( $colors_section ) ) {
			$colors_section->title = __( 'Colors & Dark Mode', 'candainfo' );
		}

		// Custom notice control.
		include_once get_theme_file_path( 'classes/class-candainfo-customize-notice-control.php' ); // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		...
		$wp_customize->add_control(
			new Canada_Info_Customize_Notice_Control(
				$wp_customize,
				'respect_user_color_preference_notice',
				array(
					'section'         => 'colors',
					...
					'active_callback' => function() {
						return 127 >= Canada_Info_Custom_Colors::get_relative_luminance_from_hex( get_theme_mod( 'background_color', 'D1E4DD' ) );
					},
				)
			)
		);
		...
		$description .= sprintf(
			...
			__( 'Dark Mode is a device setting. If a visitor to your site requests it, your site will be shown with a dark background and light text. <a href="%s">Learn more about Dark Mode.</a>', 'candainfo' ),
			esc_url( __( 'https://wordpress.org/support/article/candainfo/#dark-mode-support', 'candainfo' ) )
		);
		$description .= '</p>';
		$description .= '<p>' . __( 'Dark Mode can also be turned on and off with a button that you can find in the bottom right corner of the page.', 'candainfo' ) . '</p>';
		...
		$wp_customize->add_control(
			...
			array(
				'type'            => 'checkbox',
				'section'         => 'colors',
				'label'           => esc_html__( 'Dark Mode support', 'candainfo' ),
				'priority'        => 110,
	...
	public function html_classes( $classes ) {
		...
		$background_color            = get_theme_mod( 'background_color', 'D1E4DD' );
		$should_respect_color_scheme = get_theme_mod( 'respect_user_color_preference', false );
		if ( $should_respect_color_scheme && 127 <= Canada_Info_Custom_Colors::get_relative_luminance_from_hex( $background_color ) ) {
	...
	public function admin_body_classes( $classes ) {
		...
		if ( $current_screen->is_block_editor() ) {
			...
			if ( $should_respect_color_scheme && Canada_Info_Custom_Colors::get_relative_luminance_from_hex( $background_color ) > 127 ) {
				$classes .= ' candainfo-supports-dark-theme';
			}
		}
	...
	public function switch_should_render() {
		...
		return (
			...
			! $is_IE &&
			127 <= Canada_Info_Custom_Colors::get_relative_luminance_from_hex( get_theme_mod( 'background_color', 'D1E4DD' ) )
		);
	}
	...
	public function the_html( $attrs = array() ) {
		...
		printf(
			...
			esc_html__( 'Dark Mode: %s', 'candainfo' ),
		...
		<style>
			...
			#dark-mode-toggler > span::before {
				content: '<?php esc_attr_e( 'Off', 'candainfo' ); ?>';
			}
			#dark-mode-toggler[aria-pressed="true"] > span::before {
				content: '<?php esc_attr_e( 'On', 'candainfo' ); ?>';
			}
	...
	public function add_privacy_policy_content() {
		...
		$content = '<p class="privacy-policy-tutorial">' . __( 'Twenty Twenty-One uses LocalStorage when Dark Mode support is enabled.', 'candainfo' ) . '</p>'
				. '<strong class="privacy-policy-tutorial">' . __( 'Suggested text:', 'candainfo' ) . '</strong> '
				. __( 'This website uses LocalStorage to save the setting when Dark Mode support is turned on or off.<br> LocalStorage is necessary for the setting to work and is only used when a user clicks on the Dark Mode button.<br> No data is saved in the database or transferred.', 'candainfo' );
