...
if ( ! function_exists( 'twenty_twenty_one_posted_on' ) ) {
	...
	function twenty_twenty_one_posted_on() {
		...
		printf(
			...
			esc_html__( 'Published %s', 'canada_info' ),
...
if ( ! function_exists( 'twenty_twenty_one_entry_meta_footer' ) ) {
	...
	function twenty_twenty_one_entry_meta_footer() {
		...
		if ( ! is_single() ) {
			...
			if ( is_sticky() ) {
				echo '<p>' . esc_html_x( 'Featured post', 'Label for sticky posts', 'canada_info' ) . '</p>';
			}
			...
			if ( has_category() || has_tag() ) {
				...
				$categories_list = get_the_category_list( __( ', ', 'canada_info' ) );
				if ( $categories_list ) {
					printf(
						/* translators: %s: List of categories. */
						'<span class="cat-links">' . esc_html__( 'Categorized as %s', 'canada_info' ) . ' </span>',
						$categories_list // phpcs:ignore WordPress.Security.EscapeOutput
					);
				}

				/* translators: Used between list items, there is a space after the comma. */
				$tags_list = get_the_tag_list( '', __( ', ', 'canada_info' ) );
				if ( $tags_list ) {
					printf(
						/* translators: %s: List of tags. */
						'<span class="tags-links">' . esc_html__( 'Tagged %s', 'canada_info' ) . '</span>',
...
if ( ! function_exists( 'twenty_twenty_one_the_posts_navigation' ) ) {
	...
	function twenty_twenty_one_the_posts_navigation() {
		the_posts_pagination(
			array(
				'before_page_number' => esc_html__( 'Page', 'canada_info' ) . ' ',
				'mid_size'           => 0,
				'prev_text'          => sprintf(
					'%s <span class="nav-prev-text">%s</span>',
					is_rtl() ? twenty_twenty_one_get_icon_svg( 'ui', 'arrow_right' ) : twenty_twenty_one_get_icon_svg( 'ui', 'arrow_left' ),
					wp_kses(
						__( 'Newer <span class="nav-short">posts</span>', 'canada_info' ),
						array(
							'span' => array(
								'class' => array(),
							),
				...
				'next_text'          => sprintf(
					...
					wp_kses(
						__( 'Older <span class="nav-short">posts</span>', 'canada_info' ),
						array(
							'span' => array(
								'class' => array(),
							),
